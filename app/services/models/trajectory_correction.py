...
def detect_standing_still(seq):
    ...
    sdiff = np.pad(
        sdiff, ((0, 1), (0, 0)), mode="edge"
    )  # repeat last value of sdiff one more time so it's the same size as seq
    sdiff = np.linalg.norm(sdiff, axis=1)
    standing_still_mask = (
        sdiff < 0.007
    )  # I found this threshold from watching the values of real sequences
...
def get_perpendicular_in_dir_of_head(vec1, vec2, pose):
    ...
    right_general_direction = np.mean(
        [dir_of_right_toes, dir_of_left_toes, dir_of_head, dir_of_knee1, dir_of_knee2],
        axis=0,
    )
    if np.dot(res, right_general_direction) < 0:
        res *= -1
    return res / np.linalg.norm(res)  # vector of length 1


def get_rotation_matrix(
    vec1, vec2=np.array(([[1, 0, 0], [0, 0, 1], [0, 1, 0]])), weights=[0.8, 1, 0.6]
):
...
def transform_seq_so_it_has_straight_trajectory_AMASS(
    seq, n_frames_est_mov_dir, window_size, polynomial
):
    ...
    m_dir = np.pad(
        m_dir, ((0, n), (0, 0)), mode="edge"
    )  # Reapeat last value n times so it's the same len as seq
    ...
    for i, mov_dir_vec in enumerate(movement_direction):
        rot_mat = get_rotation_matrix(
            np.stack([mov_dir_vec, hip_vectors[i], height_vectors[i]])
        )
...
def correct_AMASS_sequence(seq):
    seq = transform_seq_so_it_has_straight_trajectory_AMASS(
        seq, n_frames_est_mov_dir=15, window_size=None, polynomial=None
    )
