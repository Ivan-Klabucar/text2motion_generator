...
class Text2Motion_GenerationPipeline:
    def __init__(self) -> None:
        ...
        self.motion_decoder.load_state_dict(
            torch.load(
                os.getenv('MOTIONCLIP_DECODER_PATH'),
                weights_only=True
            )
        )
        ...
        self.text_encoder.load_state_dict(
            torch.load(
                os.getenv('CLIP_TEXT_ENCODER_PATH'),
                weights_only=True
            )
        )
    ...
    def infer(self, request: TextPrompt) -> dict:
        with torch.no_grad():
            ...
            motion_xyz = motion_xyz.permute(0, 3, 1, 2) # Shape (1, 60, 24, 3) or (num_samples, frames, joints, coordinate axis)
